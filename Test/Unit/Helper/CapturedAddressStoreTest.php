 ...
 * session key shared with every other module on the installation, ShopperScopedSessionStores flushes
 ...
 * ShopperScopedSessionStoresTest, together with the two verdict caches that share the rule. What
...
class CapturedAddressStoreTest extends TestCase
{
    ...
    private function createShopper(?ArrayObject $session = null): array
    {
        ...
        // same customer throughout, so ShopperScopedSessionStores adopts it and never flushes.
        // The identity-change behaviour is ShopperScopedSessionStoresTest's subject, and leaving it
     ...
     * shared with every other module on the installation, ShopperScopedSessionStores deliberately
    ...
    {
        return [
            // What ShopperScopedSessionStores itself writes when it flushes the store, so this case
     ...
     * other module on the installation, ShopperScopedSessionStores deliberately writes null into it
     ...
     * in a ShopperScopedSessionStores; if it ALSO kept the raw session in a property, a later edit
    ...
    public function testTheStoreIsReachedOnlyThroughTheShopperOwnershipGuard(): void
    {
        ...
        foreach ($held as $name => $value) {
            $this->assertNotInstanceOf(
                ...
                sprintf(
                    ...
                    . 'be reachable only through ShopperScopedSessionStores, or a future edit can read and write it '
        ...
        $this->assertCount(
            1,
            $guards,
            'Helper\Controller must hold exactly one ShopperScopedSessionStores: that is the seam every access to '
