  ...
  $(document).ready(function () {
    ...
    setInterval(function () {
      ...
      if (streetInput.length === 0) {
        streetInput = $('[name*="street"][name*="[0]"]');
      }
    ...
    if ($(streetInput).length !== 0) {
      $(streetInput).each(function (index, element) {
        if ($(element).data("loqate_parsed") !== 1) {
          ...
          if (county_input.length === 0) {
            county_input = form.find('[name*="region"]');
          }

          let county_list = form.find("#region_id");
          if (county_list.length === 0) {
            county_list = form.find('[name*="region_id"]');
          }
...
<span>${buildHighlights(item)}</span><span>${
  ...
  function handleRetrieveApiResponse(response, addressElements) {
    if (Array.isArray(response)) {
      ...
      $.each(addressMapping, function (key, val) {
        if (key === "county") {
          $(addressElements[key]["input"])
            .val(autofillAddress[val])
            .change()
            .get(0)
            .dispatchEvent(evt);
          ...
          if ($(countyField).length) {
            $(countyField)
              ...
              .filter(function () {
                ...
                var region2 = autofillAddress[val]
                  .normalize("NFD")
                  .replace(/\p{Diacritic}|(-)|(\s)/gu, "");

                return (
                  $(this).val() === autofillAddress[val] ||
                  region1.toLowerCase() === region2.toLowerCase()
                );
              })
              .prop("selected", true);
  ...
  function displayError(message, container) {
    ...
    const errorElem = $(
      "<div class='loqate-error-item message error'>" + message + "</div>"
    );
